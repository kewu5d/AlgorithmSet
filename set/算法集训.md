...
39. ```cpp
    ...
    ```
       ...
       这段代码主要用于**优化输入输出流**的性能，尤其在算法竞赛或需要处理大规模数据时至关重要。
...
45. ```cpp
    ...
    > - 这是给**字面量**加后缀，**不是给变量**“加 (LL)”
    >
    >     - `ans(LL)` 在 C++ 语法里表示“调用 `ans`，传入参数 `LL`”。如果 `ans` 是普通变量，这会直接**编译错误**
    >
    > - **强转**要这么写（推荐）
    >
    >     - C 风格：
...
64. `nth_element` 是 C++ 标准模板库（STL）中一个高效的排序相关算法，主要用于**部分排序**和**快速定位特定位置**的元素。[P1923 【深基9.例4】求第 k 小的数 - 洛谷](https://www.luogu.com.cn/problem/P1923)
...
89. vscode的调试器
    ...
    - <img src="D:\Node-typora\note-picture\image-20251117170354580.png" alt="image-20251117170354580" style="zoom: 33%;" />

90. long的使用

    - | 类型        | 最小保证大小 | 常见大小 (32位系统) | 常见大小 (64位系统) | 范围示例                                               |
        | ----------- | ------------ | ------------------- | ------------------- | ------------------------------------------------------ |
        | `int`       | ≥16位        | 4字节               | 4字节               | -2,147,483,648 ~ 2,147,483,647                         |
        | `long`      | ≥32位        | 4字节               | 4或8字节            | 依平台而定                                             |
        | `long long` | ≥64位        | 8字节               | 8字节               | -9,223,372,036,854,775,808 ~ 9,223,372,036,854,775,807 |

91. 优先队列`priority_queue`，定义`<queue>`

    - 排序时间：O(logn)

    - ![image-20251124160950610](D:\Node-typora\note-picture\image-20251124160950610.png)

        > 元素类型，基础容器类型：vector，比较器

92. 初始化`vector`数组的两种快慢方法

    1. ```c++
        vector<int> a(n);//直接分配了空间，还会将n个元素全部初始化为默认值。同时可变
        for (int i = 0; i < n; i++) {
            scanf("%d", &a[i]);
        }
        ```
    ...
    2. ```C++
        ...
        for (int i = 0; i < n; i++) {
            int temp;
            cin >> temp;       // cin 较慢
            a.push_back(temp); // 可能触发多次内存重新分配和拷贝
        }
        ```
...
93. 空间复杂度
    ...
    - 在算法分析中，空间复杂度通常分为两部分：
        ...
        1. **输出空间**：存储结果必须使用的空间
        2. **额外空间（辅助空间）**：除了输入和输出之外，算法运行时额外需要的空间

        ---

        ```cpp
        vector<int> avgs(n);  // 输出数组，大小为 n → O(n)
        ll now = 0, sum = 0;  // 额外变量，只有几个 → O(1)
        ```
        ...
        因为 `avgs` 是**题目要求返回的结果**，<font color=red>无论用什么算法都必须有这个数组。</font>

        所以有时候我们说：
        - **总空间复杂度 = O(n)**（包含输出数组）
        - **额外/辅助空间复杂度 = O(1)**（不包含输出数组）
        ...
        就像问你"做这道菜需要多少容器"：
        - 如果**算上盛菜的盘子**：需要一个大盘子（O(n)）
        - 如果**只算烹饪过程中用的碗**：只需要几个小碗（O(1)）

        你的算法很高效，除了必须的输出数组外，只用了常数个变量！

94. C++中，字符串的比较**（**`<`**,** `>`**,** `<=`**,** `>=`**,** `==`**,** `!=`**）**是按字典序来进行比较的

95. python与

    - | 操作       | Python      | C++              |
        | ---------- | ----------- | ---------------- |
        | `int('1')` | 得到 1      | 得到 49（ASCII） |
        | 正确写法   | `int(s[i])` | `s[i] - '0'`     |
...
96. 字符常量
    ...
    - ```cPP
        ...
        -  `'X'` 是一个字符常量。字符常量在 C/C++ 里本质上就是一个整数（ASCII 值）。
        ...
        - 所以这行代码等价于：
            ...
            > ```
            >
            > 也就是定义了一个长度为 88 的整型数组，并且用 `{}` 初始化为全零

    - <font color=red>此方法可以用来巧妙地模拟哈希表，建立`<char, int>`之后的映射</FONT>

97. 在处理字符相关的算法题（如 LeetCode）时，`int ch[130]` 是一个非常高效的技巧：

    - **ASCII 码映射**：标准 ASCII 码范围是 0-127。定义 130 个长度可以安全地容纳所有 ASCII 字符，并留有一点余量。

    - **哈希表替代品**：

    - | **代码**               | **初始化情况**            | **元素的值**                       |
        | ---------------------- | ------------------------- | ---------------------------------- |
        | `int ch[130];`         | **默认初始化** (局部变量) | **随机残留值**（脏数据），极其危险 |
        | `int ch[130]{};`（推） | **值初始化**              | **全部为 0**                       |
        | `static int ch[130];`  | 静态/全局初始化           | **全部为 0**（由底层机制保证）     |
        | `int ch[130]={};`      | 同2                       |                                    |
...
1. 首先认清CPH评测机与平时的ide编译过程不同，平时的编译路径一律不能<font color =red>~~出现中文~~</font>

2. ```bash
    错误：‘int y1’ redeclared as different kind of entity 
    8 | int x1, y1;//正方形的左上角边，右下角（x1+ans, y1+ans） 
    		  | ^
    ```

    - 在代码里定义了变量 `y1`，但在 C/C++ 标准库里**已经有一个叫做 `y1` 的数学函数**（贝塞尔函数的一种，声明在 `<math.h>` / `<cmath>` 里）。
    - 于是编译器把你写的 `y1` 当成了函数指针，而不是一个整型变量，导致后续所有地方都出错

    解决：

    - 要用y1做变量，不要重新定义为全局变量，只适合定义为局部的临时变量

3. 变量名冲突

    - ![image-20251114152454897](D:\Node-typora\note-picture\image-20251114152454897.png)

4. define要不要加分号？

    - ![image-20251121150101092](D:\Node-typora\note-picture\image-20251121150101092.png)

5. 科学计数法`1e6`表示：本质属于是浮点数`double`类型

    - ![image-20251121150844915](D:\Node-typora\note-picture\image-20251121150844915.png)
    - 解决：用`const int Num = 1e6;`
        - 这里存在隐式转换。还是推荐写成`const int Num = 1000005;`

6. 标准命名空间std

    - ![image-20251122164237311](D:\Node-typora\note-picture\image-20251122164237311.png)
    - 这是因为`vector`定义在 `std` 命名空间中，直接使用`vector`而不加 `std::` 前缀或 `using namespace std;` 会导致编译器找不到它。

7. 强制类型转换

    - <img src="D:\Node-typora\note-picture\image-20251208142440041.png" alt="image-20251208142440041" style="zoom: 50%;" />
        ...
        - 正确写法
            ...
            > (long long)nums[l]*k
            > //2.----?
            > static_cast<long long>(nums[l]) * k
            > //3.----推荐
            > 1LL * nums[l] * k
            > ```
...
8. **常数表达式**初始化数组大小
    ...
    - <img src="D:\Node-typora\note-picture\image-20251208151231779.png" alt="image-20251208151231779" style="zoom:50%;" />
...
9. 标准命名空间
    ...
    - ![image-20251208153511548](D:\Node-typora\note-picture\image-20251208153511548.png)

10. 数组访问溢出overflow

    - <img src="D:\Node-typora\note-picture\image-20251209145704493.png" alt="image-20251209145704493" style="zoom: 50%;" />

        > l = 0时，l-1会导致数组越界，溢出

11. 字符、字符串类型严格区分------不能像py一样

        - ![image-20251215200046717](D:\Node-typora\note-picture\image-20251215200046717.png)

            > - ```
            >     std::count(temp.begin(), temp.end(), '1');
            >     ```
            >
            >     > 注意这里是单引号 `'1'`，表示字符类型 `char`，而不是双引号 `"1"`，后者是字符串字面量。
            >
            > - 因为 `std::count` 的第三个参数要求和容器元素类型一致：
            >
            >     - 如果 `temp` 是 `std::string`，元素是 `char`
            >     - 如果你传 `"1"`，它是 `const char*`，类型不匹配
            >     - 编译器尝试找合适的重载，找不到，就报错了

12. 没有为函数提供完整的返回值

        - ![image-20251215202651480](D:\Node-typora\note-picture\image-20251215202651480.png)

            > - 通过分支选择返回值，但系统觉得存在疏漏
            >
            > - 💡 提示
            >
            >     即使你认为逻辑上不可能到达某个分支，也应该提供<font color=red>兜底返回值</font>，原因：
            >
            >     1. **消除编译器警告** - 保持代码整洁
            >     2. **防御性编程** - 防止未来修改代码时引入 bug
            >     3. **未定义行为** - 在 C++ 中，非 `void` 函数不返回值是**未定义行为**，可能导致程序崩溃或产生不可预期的结果

13. 求和函数`accumulate` ，int 溢出

       - ![image-20251220190510037](D:\Node-typora\note-picture\image-20251220190510037.png)

       - ```CPP
           long long sum = std::accumulate(a.begin(), a.end(), 0); // 0 => 用 int 0进行累加，累积过程溢出
           ```

           - 将 0 改为 0LL 

14. 函数传参传的是地址的话，常用const限制，确保地址内容不变

      - ![c40df66d7f33418c751387628291b6d3](D:\Node-typora\note-picture\c40df66d7f33418c751387628291b6d3.png)

          > 语义上：在 [check](vscode-file://vscode-app/d:/Program Files/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 的签名里 [T](vscode-file://vscode-app/d:/Program Files/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 是 `const`，对 [unordered_map](vscode-file://vscode-app/d:/Program Files/Microsoft VS Code/resources/app/out/vs/code/electron-browser/workbench/workbench.html) 使用 T\[c\]会尝试插入，编译也不通过；
...
15. 数组下标溢出风险！！
     ...
     - 错误原因分析
         ...
         ```C++
         while (mp.size() == diff_cnt){
             mp[nums[l]]--;
             if (mp.find(nums[l]) == mp.end()) // 这一行永远不会成立
                 mp.erase(nums[l]);
             l++;
         }
         ```
         ...
         **为什么这会导致越界？**
         ...
         1. `mp[nums[l]]--` 之后，即使该元素的计数变成了 `0`，它依然**存在**于 `unordered_map` 中，其值仅仅是 `0`。
         2. `mp.find(nums[l])` ==会==找到这个计数为 `0` 的项，因此它永远不会等于 `mp.end()`。
         3. 结果：`mp.erase()` **从未被执行**，`mp.size()` 永远不会减少。
         4. 一旦 `mp.size() == diff_cnt` 成立，由于 `size` 无法减小，`while` 循环将变成死循环，<font color=red>`l` 会一直自增直到超出 `nums` 的索引范围。</FONT>

     - 正确：

         ```CPP
         if (mp[nums[l]] == 0) // 减成0就可以从窗口抹去
             mp.erase(nums[l]);
         ```
...
2、差分：一种和<u>前缀和</u>相对的策略，可以当做是求和的**逆运算**。给定前缀和数组sum[i]，求**对应的原数组，即**差分数组

- 一维：ans[i] = sum[i] - sum[i-1]
- 二维：**a[i] [j]** = sum[i] [j]  - sum[i] [j-1] - sum[i - 1] [j] + sum[i-1] [j-1]
	- 注意：<font color=red>在1位置开始存数</font>，避免使用0位置，计算时导致越界
...
1. 求<font color=blue>区间和</font>——前缀和
  ...
  - 一维：
  	- 每次求[l, r]的和：**ans = sum[r] - sum[<font color=red>l - 1</font>]**---------<font color=red>**动手画图去推**</font>

  - 二维：
  	- 求[x1, y1]~[x2, y2]的和：<img src="D:\Node-typora\note-picture\image-20250331151115325.png" alt="image-20250331151115325" style="zoom:33%;" />
...
2. <font color=blue>差分标记数组</font>：用于解决**元素变化问题**
	...
	- 一维：另弄一个**b**数组，进行操作：
		- b[l] **+ q**, b[<font color=red>r + 1</font>] **- q**
		- 计算sum**b**前缀和数组
	...
	- 二维：如下m个操作：**x1,y1 x2,y2 q**：把a[x1, y1]~a[x2, y2]这个矩形内的所有元素，都加上q（可正可负）
		- 1）另弄一个b[i] [j]数组，进行操作：
			...
			- b[x2 + 1] [**y1**] -= q
			- b[x2 + 1] [y2 + 1] += q
		- <img src="D:\Node-typora\note-picture\image-20250331155149363.png" alt="image-20250331155149363" style="zoom: 50%;" />
		
		    >  -q+q的操作使前缀和数组在**非黄色矩形区域**元素值，在与原数组相加合并后不发生改变
		- 2）计算sumb[i] [j]前缀和数组
		- 3）**sumb[i] [j] + a[i] [j]，即可得到m个操作后的答案数组**
...
3. [P3397 地毯 - 洛谷](https://www.luogu.com.cn/problem/P3397)
	...
	- [lg3397.cpp](Training_camp\Prefix_and_differential\lg3397.cpp) 
...
- 前缀和&差分：
    - <img src="D:\Node-typora\note-picture\image-20251114155111675.png" alt="image-20251114155111675" style="zoom:25%;" /><img src="D:\Node-typora\note-picture\image-20251114155129138.png" alt="image-20251114155129138" style="zoom: 33%;" />
...
2. 二维差分数组b--->前缀和数组sumb
    ...
    - 关键点: 
      ...
      1. 当a[i]\[j]用作计算前缀和时,<font color=red>只依赖左边和上边（前面）的值,而这些值已经被计算过了</font>

      2. 当a[i]\[j]用作二维差分，只影响右边和下边（后面）未来的前缀和的值



# 尺取法

1、尺取法：一种利用<font color=red>双指针（TwoPointers）</font>遍历获取满足条件的**区间**[^2]（<font color=red>滑动窗口</font>）的算法。
...
1. [LCR 008. 长度最小的子数组 - 力扣（LeetCode）](https://leetcode.cn/problems/2VG8Kg/description/)
  ...
  - ==题目提示==`1 <= nums[i] <= 10^5`，所以数组中均为正整数，权值一定**单增**
  - vector数组是从0开始存数的
...
2. [713. 乘积小于 K 的子数组 - 力扣（LeetCode）](https://leetcode.cn/problems/subarray-product-less-than-k/description/)
  ...
  - <font color=red>大区间合法，其中的小区间一定合法</font>。计算==r-l+1,==
   - 例计算得==区间长度为3（**右窗口不动**，滑动左窗口，则固定r右窗口的子区间个数就有3）==
     - 即固定右窗口：子区间个数 = 区间长度 = r-l+1

   - <font color=blue>不用担心重复或遗漏，因为右窗口会轮流固定在每个元素上</font>
     - <img src="D:\Node-typora\note-picture\image-20251118154625133.png" alt="image-20251118154625133" style="zoom: 25%;" />
  ...
  - l=r=0，r++扩大区间
  - 区间**合法**则**计算r-l+1(合法小区间数)**，继续扩大(不用缩小l窗口，因为小区间一定是合法的，且计算在内)
  - 扩大后**不合法**就l++，缩小区间，继续判断合法后再计算r-l+1
  - 合法再扩大，<u>直至r窗口滑到顶（越界）</u>，算法结束
...
- ==常见模板==---while
    ...
    for (int r = 0; r < n.size(); r++)
    {
        ...
        window _= n[r];//_: +-*/
        //2. 缩小窗口直至合法或者窗口长度为0！
        while (l <= r && !isValid(window))
        {
            window _= n[l++];
        }
        //3. 记录答案--提供while后，已经合法
        res += ...;
        //注意l > r时，窗口长度r-l+1 = 0不会影响答案！！
    }
    ```
    
- ⚠️ 注意：**要熟悉使用增强版代码**，用**for循环**代替普通版的while循环，**循环变量直接对应右边界**，逻辑更清晰
...
1、二分查找：以在一个**升序数组**中查找一个数为例。或者找**不小于x的第一个数（lower_bound( , , )）**等等
...
- <font color=red>二分答案的上下界[L, R]：是下一轮二分答案的区间</font>
    - 因此，r = mid - 1或者r = mid以及下界l，在**初始化和循环内**都是**需要考量**的！
...
1. [P2678 NOIP 2015 提高组\] 跳石头 - 洛谷](https://www.luogu.com.cn/problem/P2678)
  ...
  - 搬走岩石的数目x<=M，才符合题意，这时猜测的mid可能是最优解。我们要**最大化最短距离**，所以需要继续向右半边猜s，即mid。
...
- ```cpp
    bool check(){}//可以分离，也可以写在while里，注意初始化变量
    
    int l = 0, r = max_possible_ans, ans = max_or_min;
    int mid;//二分猜测答案
    while (l <= r)
    {
        mid = (l + r)/2;
...
- 拆成2的幂次方----2 4 8 16 32 64 128 256 512 1024-----><font color=red>（等于每一位的次方数）`n *= n;`</font>
    ...
    - 二进制**1有效0无效（有效即有贡献）**，拆成位数是1的次方数
- 问题：使用三进制以上会更好嘛？？？
    - 不会。二进制最佳，n = n*n。使用三进制则n= n * n * n，以此类推

3、对幂进行取模

- 注意：**某数的幂**很容易超出long long的范围，**取余**应在范围内进行

- 性质：(a **\*/+-** b) <font color = red>% c</font>  <===>  (a *<font color = red>% c</font>*) **\*/+-** (b *<font color = red>% c</font>*) <font color = red>% c</font>
...
- 第一种：根据贪心思想做出来的分析，使用**优先队列**来实现
- 第二种：**双队列模拟**
    - 两堆合并后的新的一堆插入q2队列中，且<font color=red>一定是单调递增的</font>
    - 所以最小的一堆果子一定在q1或者q2的队首，**两次**取出最小的一堆
...
3. [2611. 老鼠和奶酪 - 力扣（LeetCode）](https://leetcode.cn/problems/mice-and-cheese/description/)
    ...
    - n块奶酪都要吃完，同一块奶酪不同老鼠得分不同，第一只恰好吃k块，求最大得分
    
    **思路**：
    
    - 先假设让第二个老鼠吃所有n块奶酪，然后分析第i块奶酪如果给第一只老鼠吃，总得分会更多，得分加reward1[i] - reward2[i]，即**得分差**
    - <font color=red>弄一个得分差数组</font>
    - 我们对得分差数组进行排序，选出差距（**得分增值**）最大的k块给第一只老鼠吃
    
        > 升序排列的话，就是最后k块：n-1 ~ n-1-k，交给一号老鼠吃
    - 即可得到最大得分
    ...
    - 空间：在r1数组上原地计算得分差数组
    - 时间：实用nth_element进行排序，找到前k大的数字，喂给第一只老鼠即可
...
`std::nth_element` 是 C++ STL 中一个非常强大但经常被忽视的算法。它主要用于解决 **Top-K 问题**（即在一个序列中找到**前 K 大或前 K 小**的元素），而且效率极高。

以下是关于 `std::nth_element` 的详细教程。

---

### 1. 它的作用是什么？

想象你有一堆乱序的扑克牌，你想找到**第 K 大**的那张牌。
`nth_element` 做的事情是：**重新排列**这堆牌，使得：

1.  **第 K 个位置**（下标为 `n`）上的元素，就是如果整个数组完全排序后，该位置应有的那个元素。
2.  **在这个位置之前**的所有元素，都不大于（或不小于）这个位置的元素。
3.  **在这个位置之后**的所有元素，都不小于（或不大于）这个位置的元素。

<font color=red>**注意**</font>：它**不保证**第 K 个位置之前或之后的元素是有序的，它只保证它们在正确的一侧。
...
```cpp
// 默认版本（使用 < 比较）---找第k小的数，前面都是比它小
void nth_element(RandomIt first, RandomIt nth, RandomIt last);

// 自定义比较函数版本-----自定义找第k大的数，前面都是比它大，即前k大的数
void nth_element(RandomIt first, RandomIt nth, RandomIt last, Compare comp);
```

*   `first`: 数组（或容器）的起始迭代器。
*   `nth`: 你想要**排定位置**的那个迭代器（比如你想找第 k 小，这里就传 `begin() + k`）。
*   `last`: 数组（或容器）的结束迭代器。
*   `comp`: (可选) 自定义比较规则，比如 `greater<int>()`。

### 3. 核心原理：快速选择算法 (Quick Select)

`nth_element` 的底层实现通常基于 **快速选择 (Quick Select)** 算法，这和 **快速排序 (Quick Sort)** 非常像。

*   **快速排序**：选一个基准值 (pivot)，把小的放左边，大的放右边，然后**递归处理左右两边**。
*   **快速选择**：选一个基准值，把小的放左边，大的放右边。
    *   如果我们要找的第 K 个位置在左边，我们就**只递归处理左边**，扔掉右边。
    *   如果在右边，就**只递归处理右边**。

**时间复杂度**：
*   **平均情况**：$O(N)$。因为它每次大约扔掉一半的数据，N + N/2 + N/4 ... 收敛于 2N。
*   **最坏情况**：$O(N^2)$（极少发生，且现代 STL 实现通常有防退化机制，如 IntroSelect，保证最坏也是 $O(N \log N)$）。
*   **对比 `std::sort`**：`sort` 是 $O(N \log N)$。当 N 很大时，`nth_element` 优势巨大。

### 4. 实战图解

假设数组 `v = {3, 1, 4, 1, 5, 9, 2, 6}`，我们要找**第 3 小**的数（下标为 2，因为从 0 开始）。

调用：`nth_element(v.begin(), v.begin() + 2, v.end());`
...
*   **下标 2 的位置**变成了 `2`。如果全排序，这里确实应该是 `2`。
*   **左边** `{1, 1}` 都比 `2` 小（或相等）。
*   **右边** `{3, 5, 9, 4, 6}` 都比 `2` 大（或相等）。
*   **注意**：右边的 `3, 5, 9, 4, 6` 是乱序的，并没有完全排好序。
...
int main() {
    vector<int> v = {9, 2, 1, 5, 3, 8, 7, 4, 6};
    int k = 3; // 我们想找第 4 小的数（下标为 3）

    // 默认是升序，把第 k 小的数放到 v.begin() + k 的位置
    nth_element(v.begin(), v.begin() + k, v.end());

    cout << "第 " << k << " 位的元素是: " << v[k] << endl; // 输出 4
    
    // 验证左边都比它小
    cout << "左边的元素: ";
    for(int i=0; i<k; i++) cout << v[i] << " "; // 输出可能是 2 1 3 (乱序)
    cout << endl;
...
int main() {
    vector<int> v = {1, 5, 9, 2, 6, 3};
    int k = 2; // 找前 2 大

    // 使用 greater<int>() 进行降序排列逻辑
    // 意思是：把“最大”的 k 个元素放到数组的最前面
    nth_element(v.begin(), v.begin() + k, v.end(), greater<int>());

    // 此时，v[0] 到 v[k-1] 就是最大的 k 个数
    cout << "前 " << k << " 大的数是: ";
    for(int i=0; i<k; i++) {
        cout << v[i] << " "; 
    }
    // 输出可能是: 9 6 (或者 6 9，顺序不确定，但肯定是这两个)
...
| 需求                                             | 推荐算法               | 复杂度        |
| :----------------------------------------------- | :--------------------- | :------------ |
| 需要整个数组完全有序                             | `std::sort`            | $O(N \log N)$ |
| **只需要前 K 个最大/最小，不关心它们的内部顺序** | **`std::nth_element`** | **$O(N)$**    |
| 只需要最大/最小的那**一个**                      | `std::max_element`     | $O(N)$        |
| 只需要前 K 个有序                                | `std::partial_sort`    | $O(N \log K)$ |

在你的题目中，因为你只需要把前 K 大的差值加起来，并不在乎这 K 个差值谁先加谁后加，所以 `nth_element` 是最完美的优化选择。

## 1.1、max_element 与 min_element

1. `std::max_element` 与 `std::min_element`

这两个函数是成对出现的，逻辑高度一致：

- **`std::max_element(first, last)`**：返回指向范围 `[first, last)` 中**最大**元素的**==迭代器==**。
- **`std::min_element(first, last)`**：返回指向该范围中**最小**元素的==**迭代器**。==
...
## 2、unordered 系列（哈希表） vs 有序系列（红黑树）
...
`unordered_map` 是 C++ STL 中的**哈希表**容器，用于存储**键值对（key-value）**，提供平均 O(1) 的查找、插入、删除操作。
...
int main() {
    ...
    // ========== 插入元素 ==========
    mp[5] = 2;           // 方式1：下标操作符（最常用）
    mp.insert({3, 1});   // 方式2：insert
    mp.emplace(8, 3);    // 方式3：emplace（效率稍高）
    
    // ========== 访问元素 ==========
    cout << mp[5] << endl;      // 输出: 2
    cout << mp.at(3) << endl;   // 输出: 1（越界会抛异常）
    // ⚠️ 注意：访问不存在的 key 会自动创建！
    cout << mp[100] << endl;    // 输出: 0（自动创建 mp[100]=0）
    
    // ========== 查找元素 ==========
    //返回结果是该元素是否存在：1/0
    // 方式1：find（**推荐**）
    if (mp.find(5) != mp.end()) {
        cout << "找到了，值是: " << mp[5] << endl;
    }
    
    // 方式2：count（返回0或1）
    //⚠️⚠️mp.count() 返回的是键**是否存在**（0 或 1），而不是键对应的值。
    if (mp.count(5) > 0) {
        cout << "存在" << endl;
    }
    
    // ========== 删除元素 ==========
    mp.erase(5);         // 删除 key=5 的元素
    
    // ========== 获取大小 ==========
    cout << mp.size() << endl;   // 不同 key 的数量
    cout << mp.empty() << endl;  // 是否为空
...
// 陷阱1：下标访问会自动创建元素
if (mp[10] == 0) {  // ❌ 这会创建 mp[10] = 0
    // ...
}
// 正确做法
if (mp.find(10) == mp.end()) {  // ✅ 不会创建
    // 不存在
}
if (mp.count(10) == 0) {  // ✅ 不会创建
    // 不存在
}

// 陷阱2：遍历时删除元素
for (auto& p : mp) {
    mp.erase(p.first);  // ❌ 未定义行为！
}
// 正确做法
...
| 操作      | 语法                             | 时间复杂度 |
| --------- | -------------------------------- | ---------- |
| 插入/修改 | `mp[key] = value`                | O(1) 平均  |
| 访问      | `mp[key]` 或 `mp.at(key)`        | O(1) 平均  |
| 查找      | `mp.find(key)` / `mp.count(key)` | O(1) 平均  |
| 删除      | `mp.erase(key)`                  | O(1) 平均  |
| 大小      | `mp.size()`                      | O(1)       |
| 清空      | `mp.clear()`                     | O(n)       |

`unordered_map` 是刷题中**最常用**的数据结构之一，熟练掌握它对解决滑动窗口、计数、映射类问题非常有帮助！
...
- | 容器         | 底层结构 | 是否自动排序 | 是否允许重复 | 存储内容    | 访问方式               | 典型用途               |
    | ------------ | -------- | ------------ | ------------ | ----------- | ---------------------- | ---------------------- |
    | **set**      | 红黑树   | 是           | ❌ 不允许重复 | 只有 key    | `for(auto x : s)`      | 去重 + 有序集合        |
    | **multiset** | 红黑树   | 是           | ✅ 允许重复   | 只有 key    | `for(auto x : s)`      | 统计频次、有序多重集合 |
    | **map**      | 红黑树   | 是           | ❌ key 不重复 | key → value | `m[key]` / `it->first` | 字典、映射表           |
    | **multimap** | 红黑树   | 是           | ✅ key 可重复 | key → value | `equal_range(key)`     | 一对多映射             |
...
`std::reduce` 是 C++17 引入的算法函数，定义在 `<numeric>` 头文件中。
...
// 带初始值：10+1+2+3+4+5 = 25
int sum2 = std::reduce(v.begin(), v.end(), 10);
...
- 你代码中的用法:
    ...
    - ```c++
        // 求 cardPoints[0] 到 cardPoints[m-1] 的和----即[0, m)
...
1. 需要包含 `<numeric>` 头文件
2. 由于执行顺序不确定，二元操作应满足**交换律**和**结合律**
3. 对于浮点数，由于顺序不同可能导致微小的精度差异
...
| 引入版本       | C++17                     | C++98                       |
| 执行顺序       | **无序**（可并行）        | 严格从左到右                |
| 并行支持       | ✅ 支持                    | ❌ 不支持## C++ 版本支持情况 |
| 另：时间复杂度 | 均为O(N)，N为求和区间长度 |                             |

- 蓝桥杯环境

    > 蓝桥杯官方环境通常使用 **C++14** 或更早版本，**不支持 `std::reduce`**。

### `accumulate` 语法

```cpp
accumulate(first, last, init)           // 求和，初始值为 init = 0
accumulate(first, last, init, binary_op) // 自定义操作
```

**注意**：<font color=red>`accumulate` 必须提供初始值参数（第三个参数），而 `reduce` 可以省略。</font>

### 建议

在竞赛环境中，优先使用 `accumulate`，兼容性更好。

-----

## 6、string的切片与count函数

`std::string::substr()` —— 最标准的切片函数
...
- ```CPP
    ...
    // 从下标 0 开始取 5 个字符-----------*****substr(起始位，切片长度)******
...
- 特点
    ...
    - ✅ 安全：越界会抛异常（`std::out_of_range`）
    - ✅ 简单：最常用的切片方式
    - ✅ 支持任意长度（默认到末尾）

- **时间复杂度是 O(n)**，其中 n 是切出来的子串长度

----

C 风格字符串切片（char*）

- 如果你用的是 `char*`，可以用 `strncpy` 或手动复制：
...
- ```C
    ...
    strncpy(buf, s + 2, 5); 
    // 复制从 s 下标 2 开始的 5 个字符到 buf
    // 注意：strncpy 不会自动添加字符串结束符，需要手动设置 buf[5] = '\0';
    buf[5] = '\0';
    ```
...
- | `std::count()`         | O(n) | C++98 | ✅ 全部可用       |
    | ---------------------- | ---- | ----- | ---------------- |
    | `std::ranges::count()` | O(n) | C++20 | ❌ 很多 OJ 不支持 |

    - 它就是从 `first` 到 `last` 线性扫描一遍，比较每个元素是否等于目标值
...
- | 特性                                     | queue                     | deque                                | priority_queue                    |
    | ---------------------------------------- | ------------------------- | ------------------------------------ | --------------------------------- |
    | 全名                                     | queue                     | double-ended queue                   | priority queue                    |
    | 底层容器默认                             | deque                     | 自身就是容器（分块连续内存）         | vector（默认）                    |
    | 是否允许自定义底层容器                   | 否（固定用 deque）        | —（本身就是容器）                    | 是（可指定 vector/deque 等）      |
    | 主要用途                                 | 标准队列（先进先出 FIFO） | 双端队列（两端都能进出）             | 优先级队列（默认最大堆）          |
    | **<font color=red>队首/队尾操作</font>** | **front() / back()**      | **front() / back()**                 | **top()（只有顶部）**             |
    | 两端都可以插入/删除                      | 否（只能==后插、前删==）  | 是（两端随意）                       | 否（只能插入，删除只能删顶部）    |
    | 随机访问 []                              | 不支持                    | 支持（效率较高）                     | 不支持                            |
    | 插入元素                                 | push()（队尾）            | **push_back() / push_front()**       | push()                            |
    | 删除元素                                 | pop()（队首）             | **pop_back() / pop_front()**         |                                   |
    | <font color=red>获取元素</font>          | front() / back()          | front() / back() / operator[]        | top()                             |
    | 是否自动排序                             | 否                        | 否                                   | 是（默认大根堆）                  |
    | 时间复杂度 - 插入                        | O(1)                      | O(1)（两端均摊）                     | O(log n)                          |
    | 时间复杂度 - 删除                        | O(1)                      | O(1)（两端均摊）                     | O(log n)                          |
    | 时间复杂度 - 访问首尾                    | O(1)                      | O(1)                                 | O(1)（top()）                     |
    | 空间利用率                               | 较好（用 deque 实现）     | 一般（分块连续，略有浪费）           | 较好（vector 连续内存）           |
    | 典型使用场景                             | BFS、普通队列、任务队列   | 滑动窗口、双端操作需求、==单调队列== | Top K 问题、Dijkstra、Huffman编码 |
    | 是否线程安全                             | 都不是                    | 都不是                               | 都不是                            |
    | 可不可以为空后访问                       | front()/back() 未定义行为 | front()/back() 未定义行为            | top() 未定义行为                  |

### 单调队列用处

1、窗口最大值-最小值 ≤ limit，求最长窗口长度 or 子数组个数（==双指针+单调队列==）

- ```cpp
    // 求满足 max - min <= limit 的最长连续子数组长度
    int longestSubarray(vector<int>& nums, int limit) {
        deque<int> maxQ;  // 单调递减队列 - 维护窗口最大值
        deque<int> minQ;  // 单调递增队列 - 维护窗口最小值
        int left = 0;
        int ans = 0;
        ...
        for(int right = 0; right < nums.size(); right++) {
            ...
            // 更新答案
            ans = max(ans, right - left + 1);//or 子数组个数
        }
        return ans;
    }
    //这是不定长窗口 + 单调队列最经典的写法之一，强烈建议熟练掌握
    ```

- 板子题：[2762. 不间断子数组 - 力扣（LeetCode）](https://leetcode.cn/problems/continuous-subarrays/description/)
...
- ```c++
    ...
    //保障在任何环境下都能跑，gcc11 gcc13, windows linux等等
    ```
